...
module Outcome =
    ...
    let weight (shape: Outcome) = int shape
    ...
    let decrypt =
        tryDecrypt
        >> Option.defaultWith (fun () ->
            raise (NotSupportedException("Invalid Outcome")))
    ...
    let getDesiredShape (outcome: Outcome) (shape: Shape) : Shape =
...
let decryptedRounds (input: string array) =
    ...
    |> Array.map (fun x -> (Shape.decrypt x.[0], Outcome.decrypt x.[1]))
...
|> decryptedRounds
|> Array.map (fun (opp, outcome) -> (opp, Outcome.getDesiredShape outcome opp))
